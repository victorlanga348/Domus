  ...
  const handleCreateTask = (e: React.FormEvent) => {
    ...
    if (onAddTask) {
      onAddTask({
        ...
        schedule: `${finalFrequency} • Turno da ${taskPeriod === 'morning' ? 'Manhã' : taskPeriod === 'afternoon' ? 'Tarde' : 'Noite'}`,
        nextMember: assignedName,
        nextMemberAvatar: assignedAvatar,
        rotationId: rotationIdToUse,
          ...
          {filteredTasks.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {filteredTasks.map((task) => {
                const isRotation = Boolean(task.rotationId);
